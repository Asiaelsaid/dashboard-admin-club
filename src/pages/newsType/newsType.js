...
import EditNewsType from './editNewsType';
...
export default function NewsType() {
  ...
  const getNewsType = async () => {
    try {
      const response = await axiosInstance.get('/NewsType/GetAll')
  ...
  const deleteRow = async (id) => {
    try {
      const result = await Swal.fire({
        title: 'هل حقا تريد حذف هذا النوع ؟',
        text: '! لن تتمكن من استرجاع هذا النوع',
        icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        await axiosInstance.delete(`/NewsType/Delete?id=${id}`);

        // After successful deletion, update the state to reflect the change
        setNewsType((prevNewsType) => prevNewsType.filter((item) => item.id !== id));

        // Show success message
        Swal.fire('تم الحذف', 'تم حذف هذها النوع ', 'success');
      }
    } catch (error) {
      console.log(error);
    ...
    const handleAdd = (newItem) => {
      getNewsType([...newsType, newItem]);
    };
    const rows = newsType.map((item) => createData(item.id, item.name));
    useEffect(() => {
      getNewsType()
    }, [newsType.length])
  return (
    ...
    <Table sx={{ width: '60%', margin: "2% auto" }} aria-label="customized table">
      ...
      {editingItemId !== null && (
      <EditNewsType open={true}  onClose={handleCloseEdit} itemId={editingItemId} />
    )}
    ...
    {isAddPageOpen && (
      <AddNewsType  open={true} onClose={handleCloseAdd} onAdd={handleAdd} />
    )}
    </Table>
  </TableContainer>
  )
